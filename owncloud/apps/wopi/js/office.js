...
(function ($, OC, OCA) {
	...
	OCA.Wopi = {
				...
				if (typeof actionUrl !== 'undefined') {
					...
					if (!shareToken) {
						$.post(OC.generateUrl('/apps/wopi/token'), {
							...
							OCA.Wopi.openDoc(fileId, shareToken, actionUrl, response.token, response.expires, response.wopi_src ?? wopisrc);
						});
					} else {
		...
		openDoc: function (fileId, shareToken, actionUrlTemplate, token, expires, wopisrc) {
